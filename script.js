...
----------------------
// Animation au chargement de la page de BMC
window.addEventListener('DOMContentLoaded', (event) => {
    // Animation d'entrée du contenu principal
    gsap.from(".bmc-container", {
        duration: 1.2,
        opacity: 0,
        y: 50,
        ease: "power3.out",
    ...
    gsap.utils.toArray("h2, ul, .video-section").forEach((element, i) => {
        gsap.from(element, {
            duration: 0.8,
            opacity: 0,
            y: 30,
    ...
    // Animation au survol des vidéos
...
// Animation au scroll
window.addEventListener('scroll', () => {
    const elements = document.querySelectorAll('.bmc-text li');
    elements.forEach(el => {
        const elementTop = el.getBoundingClientRect().top;
        if(elementTop < window.innerHeight * 0.8) {
            el.style.transform = `translateX(${(window.innerHeight - elementTop) * 0.1}px)`;
        }
    });
});
